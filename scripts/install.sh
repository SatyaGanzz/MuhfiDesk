...
echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}   MUHFIDESK - INSTALLER OTOMATIS                ${NC}"
echo -e "${BLUE}=================================================${NC}"
echo ""
...
if [ -f "/etc/casaos/gateway.ini" ]; then
    ...
    if [ "$CURRENT_PORT" == "5000" ]; then
        echo -e "${YELLOW}[ACTION] CasaOS menggunakan Port 5000. Menggeser ke Port 9999...${NC}"
        
        # Backup config
        cp /etc/casaos/gateway.ini /etc/casaos/gateway.ini.bak
        
        # Ganti Port 5000 jadi 9999 pakai sed
        sed -i 's/port = 5000/port = 9999/g' /etc/casaos/gateway.ini
        # Kadang formatnya "port=5000" tanpa spasi, handle juga
        sed -i 's/port=5000/port=9999/g' /etc/casaos/gateway.ini
...
# --- STEP 2: Cek & Bersihkan Port 5000 ---
echo -e "${YELLOW}[INFO] Memastikan Port 5000 tersedia...${NC}"
...
for i in {1..5}; do
    if lsof -Pi :5000 -sTCP:LISTEN -t >/dev/null ; then
        ...
        if [ "$PROCESS_NAME" == "docker-proxy" ]; then
             echo -e "${BLUE}[INFO] Port 5000 sedang digunakan oleh Docker (Mungkin instalasi sebelumnya). Akan direstart.${NC}"
             break
        fi

        echo -e "${RED}[WARNING] Port 5000 masih digunakan oleh: $PROCESS_NAME ($PID) (Percobaan $i/5)${NC}"
        ...
        echo -e "${GREEN}[OK] Port 5000 Kosong & Siap digunakan.${NC}"
        break
    fi
done
...
if lsof -Pi :5000 -sTCP:LISTEN -t >/dev/null ; then
    ...
    PROCESS_NAME=$(ps -p $PID -o comm=)
    if [ "$PROCESS_NAME" != "docker-proxy" ]; then
        echo -e "${RED}[CRITICAL ERROR] Port 5000 MASIH digunakan oleh $PROCESS_NAME. Instalasi tidak bisa lanjut.${NC}"
...
docker rm -f muhfidesk 2>/dev/null || true
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${BLUE}   http://$IP_ADDR:5000  (Port 5000) ${NC}"
    echo ""
